...
void setup() {
  ...
  if (!sw.inBoot)
  {
    Serial.println("Running communications reliability test...");
    if (SequenceTest() < 0)
    {
      Serial.println("Communications reliability test failed.  Going into infinite loop.");
      Serial.println("Check your wiring and pull up resistors and reset to try again.");
      while (1) {
        delay(10);
      }
    }
...
void bootload()
{
  ...
  while (address < 0x1F800 * 2)
  {
    ...
    if (pagebytecount == 512)
    {
      ...
      pagebytecount = 0;
      if (sw.writeUserBuffer(0x00, pagebuffer, 512) < 0)
      {
        Serial.println("Error buffering data on Serial Wombat Chip");
        break;
      }
      if (sw.writeFlashRow(pageaddress) < 0)
      {
        Serial.println("Error writing flash on Serial Wombat Chip");
        break;
      }
      pageaddress = address;
...
int16_t SequenceTest()
{
  uint8_t resetSeq[] = {0xB4, 'R', 'E', 'S', 'E', 'T', 0x55, 0x55};

  sw.sendPacket(resetSeq);
